 ...
 *     John Rodriguez
 *     Handle event from ldb called stackforide. This picks apart stack and
...
public class PDAThread extends PDADebugElement implements IThread, IPDAEventListener {
	 ...
	 * 
	 * This gets a string from ldb like:
	 * ../../sort.lua|66|main|#../../sort.lua|55|testsorts|x|n|#../../sort.lua|10|qsort|x|l|u|f|m|t|#ldb>
     * 
	...
	public IStackFrame[] getStackFrames() throws DebugException {
		if (isSuspended()) {
			String framesData = sendRequest("stackforide");
			if (framesData != null) {
				String[] frames = framesData.split("#");
				IStackFrame[] theFrames = new IStackFrame[frames.length];
				for (int i = 0; i < frames.length; i++) {
					String data = frames[i];
					theFrames[frames.length - i - 1] = new PDAStackFrame(this, data, frames.length - i - 1);
				}
				return theFrames;
			}
		}
